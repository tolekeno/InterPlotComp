...
- d:
  ...
  Prepared MET data from \[prepare_trial_data()\] with \`multi_env =
  TRUE\`, grid-completed by \[complete_field_grid()\] for a spatial
  model, and carrying the neighbour factors added by
  \[add_neighbours()\].

- neighbour_names:

  Names of the neighbour factors, from \[add_neighbours()\].

- opts:

  Named list of fitting options: \`structure\` (one of
  \`MET_STRUCTURES\`), \`rank\`, \`spatial\`, \`nugget\`,
  \`auto_simplify\`, \`exact_se\`, \`compare_baseline\`, \`maxit\`,
  \`workspace\`, \`cinv_limit\` and an optional \`relationship\` from
  \[build_relationship()\].

- progress:

  Optional \`function(i, n, reason)\` called as the simplification
  ladder advances.
